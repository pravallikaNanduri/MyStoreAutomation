...
public class LoginSteps extends SeleniumDriver {
	...
	public void user_navigates_to_the_website_and_clicks_sign_in_browser(DataTable dataTable) {
		for(int i=0; i<dataTable.height();i++) {
			SeleniumDriver.openPage(dataTable.cell(i, 0));
	...
	public void the_Existing_user_logs_in_using_valid_Email_address_as_email(DataTable dataTable) {
		for (int i = 1; i < dataTable.height(); i++) {
			loginPageActions.alreadyRegisteredSignIn(dataTable.cell(i, 0),dataTable.cell(i, 1));
			SeleniumDriver.takeScreenshot();
		}
	...
	public void clicks_forgot_password() {
		loginPageActions.clickForgotPassword();
		SeleniumDriver.takeScreenshot();
	}
	
	@Then("user should be redirected to forgot you password page")
	public void user_should_be_redirected_to_forgot_you_password_page() {
		SeleniumDriver.takeScreenshot();
		Assert.assertEquals("Forgot your password - My Store", SeleniumDriver.getDriver().getTitle());
	}
